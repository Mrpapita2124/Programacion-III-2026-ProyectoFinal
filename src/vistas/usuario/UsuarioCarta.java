...
public class UsuarioCarta extends JPanel {
	private Font fontNombre = Fuentes.FONT_SEGOE_BOLD;
	private Font fontEmail = Fuentes.FONT_SEGOE;
	private JButton session;
	private Usuario user;
	private boolean seleccionado = false;
	
	public UsuarioCarta(Usuario user) 
	{	
		this.user = user;
		inicializarPanel();
		crearComponentes();
	...
	private void inicializarPanel() 
	{
		...
		setBackground(Colores.CARTA_FONDO);
		setOpaque(true);
		
		setBorder(BorderFactory.createCompoundBorder(
			new LineBorder(Colores.BORDE_POR_DEFECTO, 1, true),
	...
	private void crearComponentes() {
		...
		infoPanel.setBackground(Colores.CARTA_FONDO);
		...
		lblNombre.setForeground(Colores.ENCABEZADOS_PRIMARIOS);
		...
		lblEmail.setForeground(Colores.ENCABEZADOS_SECUNDARIOS);
		...
		session.setBackground(Colores.LOGIN_BOTON);
		session.setForeground(Colores.TEXTO_BOTON);
		...
		session.addMouseListener(new java.awt.event.MouseAdapter() {
			...
			public void mouseEntered(java.awt.event.MouseEvent e) {
				session.setBackground(Colores.LOGIN_BOTON_HOVER);
			}
	...
	private JLabel crearAvatar() {
		JLabel avatar = new JLabel() 
		{
			...
			protected void paintComponent(Graphics g) {
				...
				if (getIcon() == null) 
				{
					g2d.setColor(Colores.AVATAR_GRADIENTE1);
	...
	public void setSeleccionado(boolean seleccionado) 
	{
		...
		{
			setBorder(BorderFactory.createCompoundBorder(
				new LineBorder(Colores.ANILLO_ENFOQUE, 2, true),
				new EmptyBorder(11, 19, 11, 19)
			));
			setBackground(Colores.BORDE_POR_DEFECTO);
		} 
		else 
		{
			setBorder(BorderFactory.createCompoundBorder(
