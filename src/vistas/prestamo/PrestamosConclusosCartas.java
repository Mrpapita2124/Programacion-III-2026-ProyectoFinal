...
public class PrestamosConclusosCartas extends PanelPersonalizable 
{
	private Font fontTitulo = Fuentes.setFontSegoe(1,15);
	int alto;
	
	public PrestamosConclusosCartas(VentanaPrincipal ventana) 
	{
		...
		List<Prestamo> prestamos = prestamoRepo.getPrestamosConcluidosDesdeUsuario();
		...
		if (prestamos.isEmpty()) 
		{
			...
			sectionTitle.setFont(Fuentes.setFontSegoe(1, 25));
			sectionTitle.setForeground(Colores.ENCABEZADOS_PRIMARIOS);
			...
			for(int i = 0; i < prestamos.size(); i++) {
				...
				card.setBackground(Colores.CLIENTE_CARTA_FONDO);
